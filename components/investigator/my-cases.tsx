...
export default function MyCases() {
  ...
  const handleAddPerson = () => {
    ...
    if (updatedCase) {
      setSelectedCase({ ...updatedCase, persons: [...(updatedCase.persons || []), newPerson] })
    }
  }
  ...
  const handleAddExhibit = () => {
    ...
    const updatedCase = cases.find(c => c.id === selectedCase.id)
    if (updatedCase) {
      setSelectedCase({ ...updatedCase, exhibits: [...(updatedCase.exhibits || []), newExhibit] })
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Manage your assigned cases and investigations</p>
        </div>
      </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <div className="flex flex-wrap gap-4">
                      <span><strong>CR Number:</strong> {case_.crNumber}</span>
                      <span><strong>DER Number:</strong> {case_.derNumber}</span>
                    </div>
                    <div>
                      <strong>Description:</strong> {case_.description}
                    </div>
                    <div className="flex flex-wrap gap-4">
                      <span><strong>Department:</strong> {case_.department}</span>
                      <span><strong>Assigned:</strong> {new Date(case_.assignedDate).toLocaleDateString()}</span>
                      <span><strong>Deadline:</strong> {new Date(case_.deadline).toLocaleDateString()}</span>
                    </div>
                    {case_.persons && case_.persons.length > 0 && (
                      <div>
                        <strong>Registered Persons:</strong> {case_.persons.length} 
                        ({case_.persons.filter(p => p.type === 'witness').length} witnesses, 
                        {case_.persons.filter(p => p.type === 'accuser').length} accusers, 
                        {case_.persons.filter(p => p.type === 'accused').length} accused)
                      </div>
                    )}
                    {case_.exhibits && case_.exhibits.length > 0 && (
                      <div>
                        <strong>Registered Exhibits:</strong> {case_.exhibits.length}
                      </div>
                    )}
                  </div>
                </div>
          ...
          {selectedCase && (
            <div className="px-6 py-4 space-y-6">
              ...
              <div className="bg-gradient-to-r from-slate-700 to-slate-600 text-white p-4 sm:p-6 rounded-lg shadow-lg">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-slate-200">CR Number</label>
                    <Input 
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium text-slate-200">DER Number</label>
                    <Input 
              ...
              <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
                <div className="sticky top-[88px] z-10 bg-white border-b pb-2">
                  <TabsList className="grid w-full grid-cols-2 h-12 bg-slate-100">
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">Person Registration</span>
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">Exhibit Registration</span>
                      ...
                      <CardTitle className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <Button
                            ...
                            onClick={() => handlePersonFormChange('type', 'witness')}
                          >
                            <Eye className="h-4 w-4 mr-1" />
                            <span className="hidden xs:inline">Add</span> Witness
                          </Button>
                          <Button
                            size="sm"
                            variant="outline"
                            className="text-white border-white/50 hover:bg-white hover:text-slate-700 transition-all"
                            onClick={() => handlePersonFormChange('type', 'accuser')}
                          >
                            <UserCheck className="h-4 w-4 mr-1" />
                            <span className="hidden xs:inline">Add</span> Accuser
                          </Button>
                          <Button
                            size="sm"
                            variant="outline"
                            className="text-white border-white/50 hover:bg-white hover:text-slate-700 transition-all"
                            onClick={() => handlePersonFormChange('type', 'accused')}
                          >
                            <UserX className="h-4 w-4 mr-1" />
                            <span className="hidden xs:inline">Add</span> Accused
                          </Button>
                        </div>
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="pt-6 space-y-8">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <h4 className="font-semibold text-slate-700 text-lg">Demographic Information</h4>
                        </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-4">
                          <div className="h-1 w-8 bg-slate-600 rounded"></div>
                          <h4 className="font-semibold text-slate-700 text-lg">Address Information</h4>
                        </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-4">
                          <div className="h-1 w-8 bg-slate-600 rounded"></div>
                          <h4 className="font-semibold text-slate-700 text-lg">Additional Information</h4>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                          ...
                          <div className="sm:col-span-2 lg:col-span-2">
                            <Button 
                              ...
                              disabled={!personForm.type || !personForm.fullName}
                            >
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center gap-2 mb-4">
                            ...
                            <h4 className="font-semibold text-slate-700 text-lg">
                              Registered Persons ({selectedCase.persons.length})
                            </h4>
                          </div>
                            ...
                            <table className="w-full border border-gray-200 rounded-lg overflow-hidden">
                              <thead>
                                <tr className="bg-slate-50">
                                  <th className="border-r border-gray-200 p-4 text-left font-semibold text-slate-700">Type</th>
                                  <th className="border-r border-gray-200 p-4 text-left font-semibold text-slate-700">Name</th>
                                  <th className="border-r border-gray-200 p-4 text-left font-semibold text-slate-700">ID Number</th>
                                  <th className="border-r border-gray-200 p-4 text-left font-semibold text-slate-700">Phone</th>
                                  <th className="p-4 text-center font-semibold text-slate-700">Actions</th>
                                </tr>
                              </thead>
                              <tbody>
                                {selectedCase.persons.map((person, index) => (
                                  <tr key={person.id} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-25'}>
                                    <td className="border-r border-gray-200 p-4">
                                      <Badge className={`${getPersonTypeColor(person.type)} font-medium`}>
                                        {person.type.charAt(0).toUpperCase() + person.type.slice(1)}
                                      </Badge>
                                    </td>
                                    <td className="border-r border-gray-200 p-4 font-medium">{person.fullName}</td>
                                    <td className="border-r border-gray-200 p-4 text-gray-600">{person.residentId}</td>
                                    <td className="border-r border-gray-200 p-4 text-gray-600">{person.phoneNumber}</td>
                                    <td className="p-4">
                          ...
                          <div className="lg:hidden space-y-4">
                            {selectedCase.persons.map((person) => (
                              <Card key={person.id} className="shadow-sm border-l-4 border-l-teal-500">
                                <CardContent className="p-4">
                                  ...
                                  <h5 className="font-semibold text-lg mb-2">{person.fullName}</h5>
                                  <div className="space-y-1 text-sm text-gray-600">
                                    <p><span className="font-medium">ID:</span> {person.residentId}</p>
                                    <p><span className="font-medium">Phone:</span> {person.phoneNumber}</p>
                  ...
                  <Card className="shadow-lg border-t-4 border-t-slate-600">
                    ...
                    <CardContent className="pt-6 space-y-8">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 mb-4">
                          <div className="h-1 w-8 bg-slate-600 rounded"></div>
                          <h4 className="font-semibold text-slate-700 text-lg">Exhibit Information</h4>
                        </div>
                      ...
                      {selectedCase.exhibits && selectedCase.exhibits.length > 0 && (
                        <div className="space-y-4">
                          <div className="flex items-center gap-2 mb-4">
                            <div className="h-1 w-8 bg-teal-600 rounded"></div>
                            <h4 className="font-semibold text-slate-700 text-lg">
                              Registered Exhibits ({selectedCase.exhibits.length})
                            </h4>
                          </div>
                          
                          <div className="grid gap-4">
                            {selectedCase.exhibits.map((exhibit) => (
                              <Card key={exhibit.id} className="border-l-4 border-l-teal-500 shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    <div className="flex-1 space-y-3">
                                      ...
                                      <div className="space-y-2 text-sm">
                                        <p className="text-gray-700">
                                          ...
                                          <span className="ml-1">{exhibit.description}</span>
                                        </p>
                                        <p className="text-gray-700">
                                          <span className="font-medium text-slate-700">Related to:</span> 
                                          <span className="ml-1">{exhibit.relatedPersonName}</span>
                                        </p>
                                        <p className="text-xs text-gray-500">
                                          <span className="font-medium">Registered:</span> 
                                          <span className="ml-1">{new Date(exhibit.registeredDate).toLocaleDateString()}</span>
                                        </p>
                                      </div>
                                    </div>
